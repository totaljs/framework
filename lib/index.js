...
function Framework() {
	...
	this.resource = function(file, name, defaultValue) {		
		...
		var resource = self.cacheRead('resources.' + file, function(key) {
			...
			resource = JSON.parse(fs.readFileSync(fileName, encoding).toString());
			return self.cacheWrite(key, resource, new Date().add('h', 1));
		});
	...
	this.onStaticDefault = function (req, res) {
		...
		if (ext === 'js' || ext === 'css') {
			fileCompile = '.' + self.config.directoryTMP + fileName.replace(/\//g, '-').substring(2) + 'compile';
			if (fs.existsSync(fileCompile))
				data = fs.readFileSync(fileCompile, encoding);
		}
		...
		if (data == null) {
			...
			if (ext === 'js') {
				...
				fs.writeFileSync(fileCompile, data, encoding);
			}
...
function Controller(app, req, res, flags, repository, isChild, layout) {
	this.app = app;
	this.req = req;
	this.res = res;
	this.flags = flags;
	...
	this.layout = typeof(layout) === 'undefined' ? app.config.defaultLayout : layout;
	...
	// renderovanie template-u ako grid
	this.template = function(visible, name, model, nameIfEmpty) {
		
		if (!visible)
			return '';

		var value = template(name, model);
		
		if (value.length === 0 && typeof(nameIfEmpty) != 'undefined')
			return template(nameIfEmpty, null, true);

		return value;
	};

	// renderovanie template-u podľa podmienky
	this.templateIf = function(bool, ifTrueName, ifFalseName, model, nameIfEmpty) {
		
		var name = (bool ? ifTrueName : ifFalseName) || '';
		var empty = nameIfEmpty || '';

		if (name.length === 0)
			return '';

		return self.template(true, name, model, nameIfEmpty);
	};

	// renderovanie existujúceho controllera
	this.controller = function(url, flags, model, layout) {

		var route = self.app.routeLookup(self.req, self.res, url, flags);
		if (route == null)
			return '';

		return route.onExecute.apply(new Controller(self.app, req, res, flags, model || self.repository, true, layout || '')) || '';
	};
	...
	this.json = function(obj) {
		...
		if (!self.isChild)
			self.app.returnContent(self.req, self.res, self.status, value, 'application/json', true);
	...
	this.binary = function(data, contentType) {
		self.app.returnContent(self.req, self.res, self.status, data, contentType || 'application/octet-stream', false);
	};
	...
	// render template
	function template(name, model, isEmpty) {
		...
		filename = '.' + path.join(config.directoryTemplates, internal.extension(filename, '.html'));
		if (fs.existsSync(filename))
		...
		return templater.renderTemplate(value, model, function(value) {
			if (isPlain)
				return value;
