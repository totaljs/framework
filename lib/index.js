...
function Framework() {
	...
	this.returnFile = function(req, res, fileName, downloadName) {
		...
		headers['Etag'] = etag;
		headers['Content-Type'] = utils.getContentType(extension.substring(1));
		...
		if (compress) {
			if (accept.indexOf('deflate') != -1) {
				zlib.deflate(new Buffer(contentBody), function deflate(err, data) {
					...
					res.writeHead(code, { 'Content-Type':  contentType, 'Content-Encoding': 'deflate' });
			...
			if (accept.indexOf('gzip') != -1) {
				zlib.gzip(new Buffer(contentBody), function gzip(err, data) {
					...
					res.writeHead(code, { 'Content-Type': contentType, 'Content-Encoding': 'gzip' });
					res.end(data, encoding);
				});
				return;
			}
...
function Subscribe(framework, req, res) {
	...
	this.returnContent = function(contentBody, contentType) {
		if (self.isFlush())
			return;

		self.app.returnContent(self.req, self.res, 200, contentBody, contentType, true);
	};

	this.returnFile = function(fileName, contentType, downloadName) {

		if (self.isFlush())
			return;

		var fileName = utils.combine(self.options.directoryPublic, fileName);
		self.app.returnFile(self.req, self.res, fileName, downloadName);
	};
	...
	this.onLookup = function(obj, options) {
  		...
  		try
  		{
  			...
  			obj.onExecute.call(controller.load(obj.name, self, self.req, self.res, options));
  		} catch (err) {
